...
*/
-- SM Maybe add note that only "Main" course "Main" dish should be in <b></b> tags. And "Main" course should also show main dish or side dish.
-- Easiest way would be using concat() with multiple case statements as shown.
...
*/
SELECT distinct ReversedRecipe = concat(upper(substring(reverse(r.RecipeName), 1,1)), lower(SUBSTRING(reverse(r.RecipeName), 2,LEN(r.recipename)))), 
...
*/
-- SM You should return "one" recipe name. "First" ingredient of "each" recipe in system. And "last" step of "each" recipe in system.
;with x as(
    select r.RecipeId, Step = max(rs.SequenceVal), Ingredient = min(ri.SequenceVal)
    from Recipe r
    join RecipeIngredient ri
    on r.RecipeId = ri.RecipeId
    join RecipeStep rs
...
-- SM I added measurement but no need.
select RecipeName = 'Chocolate Chip Cookies', Ingredient = concat(ri.MeasurementQty, ' ', m.MeasurementType, ' ', i.Ingredient), rs.Instruction
from x
join RecipeStep rs
...
-- SM Update question to "For each user show how many recipes they created and what status it's in. Show 0 if none."
...
-- SM Show all cuisines, and for one user show how many recipes that user created.
;with x as(
...
select s.Username, c.CuisineType, NumOfRecipes = count(x.RecipeID)
from Cuisine C 
left join x 
